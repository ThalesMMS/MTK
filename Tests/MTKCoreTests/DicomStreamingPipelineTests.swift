...
final class DicomStreamingPipelineTests: XCTestCase {
    func test_loadVolumeStreamingEmitsRawSlicesAndMetadataWithoutCPUHUConversion() throws {
        ...
        XCTAssertEqual(success.metadata.spacing.x, 1.0, accuracy: 0.000_000_01)
        XCTAssertEqual(success.metadata.spacing.y, 2.0, accuracy: 0.000_000_01)
        XCTAssertEqual(success.metadata.spacing.z, 3.0, accuracy: 0.000_000_01)
        XCTAssertEqual(success.metadata.recommendedWindow, WindowLevelPresetLibrary.softTissue.windowRange)
        XCTAssertEqual(success.metadata.imageData.pixelFormat, .int16Signed)
        XCTAssertEqual(success.metadata.imageData.spacing, VolumeSpacing(x: 1, y: 2, z: 3))
        XCTAssertEqual(success.metadata.imageData.clinicalMetadata?.sourcePixelFormat, .int16Signed)
