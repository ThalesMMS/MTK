...
public final class VolumeCubeMaterial: SCNMaterial, SCNProgramDelegate {
    ...
    /// Buffer consumido pelo shader `volume_fragment` (ver `VolumeRendering.metal`).
    ...
    private var datasetIntensityRange: ClosedRange<Int32> = (-1024)...3071
    ...
    public func setHuWindow(_ window: HuWindowMapping) {
        ...
        uniforms.tfCoordMin = window.tfMin
        uniforms.tfCoordMax = window.tfMax
        pushUniforms()
    }
    ...
    public func setHuWindow(minHU: Int32, maxHU: Int32) {
        let mapping = VolumeCubeMaterial.makeHuWindowMapping(
            minHU: minHU,
            maxHU: maxHU,
            datasetRange: datasetIntensityRange,
...
private extension VolumeCubeMaterial {
    func apply(factory: VolumeTextureFactory,
        ...
        uniforms.tfCoordMin = mapping.tfMin
        uniforms.tfCoordMax = mapping.tfMax
...
private extension VolumeCubeMaterial {
    ...
    func updateTransferFunctionDomain() {
        ...
        if transferFunctionDomain == nil {
            ...
            transferFunctionDomain = ClosedRange(
                uncheckedBounds: (
                    lower: Float(datasetIntensityRange.lowerBound),
                    upper: Float(datasetIntensityRange.upperBound)
                )
            )
        }
    }
