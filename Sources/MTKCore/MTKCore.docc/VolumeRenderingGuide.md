...
#### Shader-level ZSKIP heuristic

The shader-level ZSKIP heuristic is always active in DVR mode. It skips consecutive transparent samples during ray marching and has no MPS dependency.
...
#### MPS acceleration structure

The optional MPS acceleration structure is a precomputed min-max pyramid generated by ``MPSEmptySpaceAccelerator``. It requires Metal Performance Shaders and is unavailable on devices where MPS is not supported. When available, it augments the Metal ray marcher and can provide 30%+ speedup for sparse volumes.
