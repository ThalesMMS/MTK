...
inline float normalize_value_int(int value, int minValue, int maxValue) {
...
inline float normalize_value(short value, int minValue, int maxValue) {
    return normalize_value_int(int(value), minValue, maxValue);
}

inline float normalize_value(ushort value, int minValue, int maxValue) {
    return normalize_value_int(int(value), minValue, maxValue);
}
...
inline float3 compute_gradient(texture3d<ushort, access::read> volume,
                               uint3 coord,
                               uint3 dimension) {
    if (dimension.x < 2 || dimension.y < 2 || dimension.z < 2) {
        return float3(0.0f);
...
kernel void computeGradientHistogramThreadgroupUnsigned(texture3d<ushort, access::read> inputTexture [[texture(0)]],
                                                        constant uint &intensityBinCount [[buffer(0)]],
                                                        constant uint &gradientBinCount [[buffer(1)]],
                                                        constant int &voxelMin [[buffer(2)]],
                                                        constant int &voxelMax [[buffer(3)]],
...
kernel void computeGradientHistogramLegacyUnsigned(texture3d<ushort, access::read> inputTexture [[texture(0)]],
                                                   constant uint &intensityBinCount [[buffer(0)]],
                                                   constant uint &gradientBinCount [[buffer(1)]],
                                                   constant int &voxelMin [[buffer(2)]],
                                                   constant int &voxelMax [[buffer(3)]],
