...
/// Abstraction protocol for Metal-native presentation surfaces hosted by MTKUI.
///
/// Implementers provide a platform view plus display scale management. The
/// clinical presentation contract remains GPU-native; interactive frame
/// presentation must route through `MTKView`/`CAMetalLayer` presentation code
/// rather than `CGImage` display glue.
@MainActor
public protocol ViewportPresenting: AnyObject {
...
public struct ViewportPresentingView: UIViewRepresentable {
    public final class ContainerView: UIView {
        private weak var hostedView: UIView?
        private let logger = Logger(subsystem: "com.isis.viewer", category: "ViewportPresentingView")
        private var lastLoggedSize: CGSize = .zero
        private var lastLoggedViewSize: CGSize = .zero
        ...
        public func host(_ view: UIView) {
            ...
            logger.debug("ViewportPresentingView hosting view type=\(typeName) currentSuperview=\(String(describing: type(of: view.superview))) self=\(String(describing: type(of: self)))")
        ...
        private func logHostingEvent(view: UIView) {
            ...
            guard size != lastLoggedSize || viewSize != lastLoggedViewSize else { return }
            lastLoggedSize = size
...
public struct ViewportPresentingView: NSViewRepresentable {
