...
//  Isis DICOM Viewer
//
//  Atua como fachada sobre os pipelines Metal do renderizador volumétrico, cuidando de compilar shaders, carregar recursos e produzir imagens de fallback.
//  Garante compatibilidade com diferentes GPUs, cacheia pipelines de fragmento e compute, e expõe sinalização sobre suporte a Metal Performance Shaders.
...
public final class MetalRaycaster {
    ...
    private let logger = Logger(subsystem: "com.isis.metalvolumetrics",
                                category: "Volumetric.MetalRaycaster")
    ...
    public init(device: any MTLDevice,
        ...
        if let library {
            ...
            resolvedLibrary = defaultLibrary
                  ...
                  let bundleLibrary = try? device.makeDefaultLibrary(bundle: .module) {
    ...
    public func makeFragmentPipeline(colorPixelFormat: MTLPixelFormat,
        ...
        descriptor.label = label ?? "MetalRaycaster.Volume.Fragment"
    ...
    public func load(dataset: VolumeDataset) throws -> DatasetResources {
        ...
        guard let texture = factory.generate(device: device) else {
            ...
            logger.error("Failed to create volume texture for dataset dim=\(dims.width)x\(dims.height)x\(dims.depth)")
    ...
    @discardableResult
    public func loadBuiltinDataset(for part: VolumeCubeMaterial.BodyPart) throws -> DatasetResources {
        let factory = VolumeTextureFactory(part: part)
        guard let texture = factory.generate(device: device) else {
            logger.error("Failed to create built-in dataset for part: \(part.rawValue)")
            throw Error.datasetUnavailable
        }
        let resources = DatasetResources(
    ...
    func makeCommandBuffer(label: String? = nil) -> (any MTLCommandBuffer)? {
        let commandBuffer = commandQueue.makeCommandBuffer()
        commandBuffer?.label = label ?? "MetalRaycaster.CommandBuffer"
