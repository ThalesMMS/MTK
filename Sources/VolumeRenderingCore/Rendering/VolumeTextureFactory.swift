...
//  Isis DICOM Viewer
//
//  Responsável por converter datasets volumétricos em texturas Metal 3D reutilizáveis
//  pelos materiais de volume e MPR. Normaliza dimensões, espaçamentos e formatos
//  de pixel para o runtime de renderização e mantém utilidades para carregar presets
...
public final class VolumeTextureFactory {
    public private(set) var dataset: VolumeDataset
    private let logger = Logger(subsystem: "com.isis.metalvolumetrics",
                                category: "Volumetric.VolumeTextureFactory")
    private static let resourceLogger = Logger(subsystem: "com.isis.metalvolumetrics",
                                               category: "Volumetric.VolumeTextureFactory")
    ...
    public convenience init(part: VolumeCubeMaterial.BodyPart) {
...
private extension VolumeTextureFactory {
    ...
    static func loadDataset(fromArchiveAt url: URL,
        ...
        do {
            ...
            resourceLogger.error("Failed to extract archive at \(url.lastPathComponent): \(String(describing: error))")
            return placeholderDataset()
        }
